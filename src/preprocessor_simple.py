"""
preprocessor_simple.py - Loads both .xlsx and .xls files
"""
...
class SimpleDataPreprocessor:
    """
    Loads Excel files in both .xlsx and .xls formats from XM
    """
    ...
    def load_excel_file(self, filepath):
        """Load a single Excel file (supports .xlsx and .xls)"""
        try:
            # Determine file type and use appropriate engine
            file_str = str(filepath)
            
            if file_str.endswith('.xls') and not file_str.endswith('.xlsx'):
                # For old Excel 97-2003 format
                print(f"  Detectado formato antiguo .xls para {Path(filepath).name}")
                df = pd.read_excel(filepath, skiprows=3, engine='xlrd')
            else:
                # For modern .xlsx format
    ...
    def get_last_year(self):
        """Get the last year available in the data"""
        data = self.load_all_files()
        if data is not None and len(data) > 0:
            return data['date'].max().year
        return None
